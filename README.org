...
The keys for changing the brush (=-,+,#,~,<return>,<delete>=) can now be
prefixed by the =INS= key, like any other uniline functions.

Those keys can keep the meaning they had before entering uniline-mode
(usually self insertion).

Also, the compilation is more robust when Hydra is not installed.

See [[#brush-style][Brush style]].
...
MacOs. Sometimes it is bound to =M-n=, but not always.
See [[#keyboard-bindings][Keyboard bindings]] for how to bind tilde =~= and other keys.
