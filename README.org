...
Uniline is now connected to the =abbrev= standard Emacs abbreviation
system. This enables permanently storing small diagrams for latter
use. See [[#abbrev][Abbrev]].
...
- [[#gallery-pure-unicode-diagrams-in-emacs][Gallery: pure UNICODE diagrams in Emacs]]
  ...
  - [[#replace-words-with-simple-sketches][Replace words with simple sketches]]
  - [[#visualize-floating-point-layouts][Visualize floating point layouts]]
  - [[#illustrate-proportions][Illustrate proportions]]
  - [[#3d-illusions][3D illusions]]
  - [[#phylogeny-hierarchy][Phylogeny hierarchy]]
  - [[#pure-text][Pure text]]
  - [[#beware][Beware!]]
- [[#a-minor-mode-for-drawing][A minor mode for drawing]]
  ...
  - [[#basic-brushes][Basic brushes]]
  - [[#the-25-dimensions-brushes][The 2.5 dimensions brushes]]
...
- [[#macros][Macros]]
- [[#abbrev][Abbrev]]
  - [[#create-an-abbrev][Create an abbrev]]
  - [[#insert-an-abbrev][Insert an abbrev]]
  - [[#manage-abbrevs][Manage abbrevs]]
  - [[#library-of-abbrevs][Library of abbrevs]]
...
- [[#customization][Customization]]
  ...
  - [[#prefix-brush-settings-with-ins][Prefix brush settings with INS]]
...
- [[#how-uniline-behaves-with-its-environment][How Uniline behaves with its environment?]]
  ...
  - [[#compatibility-with-abbrev][Compatibility with abbrev]]
...
By the way, if you want to understand this mysterious equation, listen
to the excellent Mahesh Shenoy explaining on his channel how to
rebuild it from scratch: https://www.youtube.com/watch?v=3QU-_PSbKlo
...
** Replace words with simple sketches
:PROPERTIES:
:CUSTOM_ID: replace-words-with-simple-sketches
:END:

[[file:images/music-player.png]]
...
** Visualize floating point layouts
:PROPERTIES:
:CUSTOM_ID: visualize-floating-point-layouts
:END:

[[file:images/floating-point-layout.png]]
...
#+begin_example
  ...
  ╰─╯╰─┴─┴─┴─┴─┴─┴─┴─╯╰─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─╯
   ...
   │    ╰────────╯                     ╰────────╯
   ╰─╮sign╭╴
     ╰────╯           IEEE 754-2008 32 bits floating point
                      ╺━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╸
...
:PROPERTIES:
:CUSTOM_ID: illustrate-proportions
:END:

[[file:images/rulers.png]]
...
** 3D illusions
:PROPERTIES:
:CUSTOM_ID: 3d-illusions
:END:

The 3D brushes give the illusion of a light casting shadows. Depending
on whether the shape is drawn clock-wise or anti-clock-wise, it gives
the illusion it is etched out or hollow.

[[file:images/etched-in-out.png]]
...
[[file:images/connected-3D-boxes.png]]

#+begin_example
 ╭───────╮       ╭───────┒          ╭───────╮       ╭───────╮
 │ unua  ┃   ╭──▶│ mikso ┃          │ unua  ▐   ╭──▶│ mikso ▐
 │       ┠───╯   ┕━━┑    ┖─╮        │       ▐───╯   ╰▄▄▖    ▝─╮
 ╰━━┯━━━━┛          │      ┠────□   ╰▄▄▄▄▄▄▄▟          │      ▐────□
    │               │      ┃──╮        │               │      ▐──╮
    │ ╭─────╮       │    ┏━┛  ╰─■      │ ╭─────╮       │    ▗▄▟  ╰─■
...
** Phylogeny hierarchy
:PROPERTIES:
:CUSTOM_ID: phylogeny-hierarchy
:END:

[[file:images/phylogeny.png]]
...
If the customizable variable =uniline-prefix-for-setting-brush= is
non-nil, then those keys must be prefixed by =INS= to change the brush
(the =INS= key is customizable, see [[#insert-key][Insert key]]). Otherwise, they keep
the meaning they had before entering uniline-mode (usually self
insertion).
...
** The 2.5 dimensions brushes
:PROPERTIES:
:CUSTOM_ID: the-25-dimensions-brushes
:END:

12 composite brushes are available. (They are composite in the sense
that they combine elementary brushes). They are called 2.5D, aka 2.5
dimensions, because they give the illusion of a foreground shape which
casts its shadow on the background.

Type: ~<insert> b~, then one of the 12 shortcut letters.

The selected brush is displayed in the lighter, at the bottom of the
Emacs screen.

The "light" can be choosen to come
- from upper left and cast "shadows" to lower right,
- or from upper right and cast "shadows" to lower left.
...
#+begin_example
   ...
   ┏━━━▽━━━━▽━━━━▽━━━━▽━━┓
   ┃ ╭──╮╭──╮  ▖──╮╭──▗  ┃
   ┃ ▌a ││b ▐  ▌A ││B ▐  ◁─╴block shadow
   ┃ ▙▄▄╯╰▄▄▟  ▙▄▄▖▗▄▄▟  ┃
   ┃                     ┃
   ┃ ╭──╮╭──╮  ╓──╮╭──╖  ┃
   ┃ ║c ││d ║  ║C ││D ║  ◁─╴double shadow
   ┃ ╚══╯╰══╝  ╚══╛╘══╝  ┃
   ┃                     ┃
   ┃ ╭──╮╭──╮  ┎──╮╭──┒  ┃
   ┃ ┃e ││f ┃  ┃E ││F ┃  ◁─╴thick shadow
   ┃ ┗━━╯╰╼━┛  ┗━━┙┕━━┛  ┃
   ┗━━━△━━━━△━━━━△━━━━△━━┛
       │    │    │    │
       │    ╰────├────╰──╴light comes
...
When a shape is drawn in the clock-wise direction, it appears
etched-out, as if it was on the foreground.

When drawn anti-clockwise, it appears as a hole.

Both rotation directions can be combined for interesting effects.
...
The rectangle drawing and the contour tracing algorithms operate
clock-wise, thus creating etched out shapes.
...
(Note: the =<insert>= key is customizable, see [[#insert-key][Insert key]]).
...
No =<insert>= key on the *MacOs keyborad*? Your =<insert>= is *already used
for something else*?

No problem! Through customization, another key may be defined instead
of =<insert>=. Type:
...
Actually, there is no obvious replacement for the =<insert>= key on
MacOS. Therefore, no default is provided.

See here https://convivialcomputing.net/posts/2026-07-03-uniline.html
how a new user of Uniline handled the missing =<insert>= key on her-his
MacOs. Hopefully Uniline became more Mac friendly than a few months ago
...
(The =<insert>= key may be customized, see [[#insert-key][Insert key]]).
...
(The =INS= key may be customized, see [[#insert-key][Insert key]]).
...
Select a region, press =<insert>= (this key may be customized, see
[[#insert-key][Insert key]]), then:
- =c= to copy
- =k= to kill
- =y= to yank (aka paste)
...
+ =+=: thin lines and intersections become thick, empty glyphs get
  filled. For instance, the empty square □ gets filled ■.
+ =-=: thick lines and intersections become thin, filled glyphs are
  emptied. For instance, the filled arrow ▶ becomes empty ▷.
...
** UNICODE to ASCII
:PROPERTIES:
:CUSTOM_ID: unicode-to-ascii
:END:

After drawing a diagram with Uniline, it is possible to convert it to ASCII.

Select a rectangle, then type =INS s A=.

The diagram can be converted back to UNICODE by pressing =0= or
=a=. However this is a lossy transformation, because ASCII was never
intended to draw diagrams. The ASCII → UNICODE filter makes good
guesses thought.
...
(The =<insert>= key is customizable, see [[#insert-key][Insert key]]).
...
The contour algorithm works nicely with 2.5D brushes. An external
contour appears etched out, while an internal contour appears as a
hole. Example:

[[file:images/contour-etched-in-out.png]]
...
- The external contour was achieved by putting the cursor on "E" and typing =INS c=.
- The internal one with the cursor on "I" and also typing =INS c=.

This works because the contour algorithm follows the shape clockwise
for an external path, and anti-clockwise for an internal one. There is
a single algorithm, which goes forward by keeping its right hand
touching the wall.
...
:CUSTOM_ID: abbrev
:END:
The =abbrev= standard Emacs system stores snippets for latter use.  Give
a snippet a name. When this exact name appears in a buffer, it is
automatically replaced by the snippet. This happens by typing a blank
following the name under the abbrev minor mode, or typing =C-x '=.

A Uniline-abbrev-snippet is a rectangle. (This contrast with usual
abbrevs, which are contiguous snippets of text, maybe on several
lines). An abbrev rectangle may be overlapped anywhere in the Uniline
buffer, pretty much like yanking a rectangle.

** Create an abbrev
:PROPERTIES:
:CUSTOM_ID: create-an-abbrev
:END:

Under =uniline-mode= create an abbrev:
- select a rectangular region,
- type =<insert> a=,
- answer to the prompt for a new name.

That's all.

** Insert an abbrev
:PROPERTIES:
:CUSTOM_ID: insert-an-abbrev
:END:

To insert a previously stored abbrev:
- type the name of the abbreviation in the buffer under =uniline-mode=,
- either type =C-x '=,
- or a space if =abbrev-mode= is active.
- the name is replaced by the stored rectangular diagram.

Uniline abbreviations are kept under the =uniline-abbrev-table=. They
are available only under the Uniline minor mode.

** Manage abbrevs
:PROPERTIES:
:CUSTOM_ID: manage-abbrevs
:END:

To see, change, edit, and manage the abbreviations:
- type =M-x list-abbrevs=,
- look for the =(uniline-abbrev-table)=,
- edit at will,
- permanently store by typing =C-x C-s=.

** Library of abbrevs
:PROPERTIES:
:CUSTOM_ID: library-of-abbrevs
:END:

As mentioned by a user (see
https://www.reddit.com/r/emacs/comments/1vpvld9/uniline_goes_25_dimensions/),
a library of usual diagram elements is handy. They can be
representations of a valve, a motor, a vu-meter, a switch, a resistor,
a cat, a hat, or whatever makes sense for a particular kind of
diagram.

It is easy to create your own. If you would like to contribute some
specialized library (electric engineering or process orchestration for
instance), you are welcome. There is not yet a repository for such a
library. But this does not stop from starting one.
...
** Prefix brush settings with INS
:PROPERTIES:
:CUSTOM_ID: prefix-brush-settings-with-ins
:END:
To change the brush, use any of the keys ~+~, ~-~, ~=~, ~#~, =~=,
~<delete>~, ~<return>~ as explained in [[#brush-style][Brush style]].

Those keys may or may not be prefixed by the =INS= key.

Variable =uniline-prefix-for-setting-brush=.

When non-nil, this variable requires those keys to be prefixed by
=INS=. Doing so frees the keys, so they can retain the meaning they
had before entering uniline-mode (usually self insertion).

** Keyboard bindings
:PROPERTIES:
:CUSTOM_ID: keyboard-bindings
:END:
Suppose you want to change the tilde key to some other key. Uniline
binds the =~= tilde key to the =uniline-set-brush-dot-toggle=
function. (This function toggles the brush to 3 styles of lines:
coarse dots, fine dots, plain lines, and again.)

On MacOs, =~= is not always readily available. So, you may want to use
=M-n= instead (which happens to sometime insert a tilde on MacOs, but
not always). To do so, the =bind-key= or =keymap-set= functions are
handy:

#+begin_src elisp
;; do this:
(bind-key "M-n" 'uniline-set-brush-dot-toggle uniline-mode-map)
;; or that:
(keymap-set uniline-mode-map "M-n" 'uniline-set-brush-dot-toggle)
#+end_src

One of those instructions can be written in your =.emacs= file. But
beware that Uniline must already have been loaded for =uniline-mode-map=
to be defined.

One way is to delay those instructions until Uniline is loaded. Example:

#+begin_src elisp
(eval-after-load 'uniline-core
  '(bind-key "M-n" 'uniline-set-brush-dot-toggle uniline-mode-map))
#+end_src

Another way is to delegate time dependencies to =use-package=. Here are
two variants, =:config= and =:bind :map=:
...
(use-package uniline-hydra
  ...
  :bind (:map uniline-mode-map
              ("M-n" . uniline-set-brush-dot-toggle)))))
#+end_src

Look at the =uniline-mode-map= variable to see the top-level key-bindings that
Uniline defines (locally and while in =uniline-mode=).
...
For huge contours, it is always possible to launch the contour command
=<insert> c= once again, from where the last command stopped.
...
:END:
The cursor is hollow by default, so that what is under the cursor
remains visible.
...
Even when turned off, the welcome message can still be displayed by
pressing =C-h TAB=.
...
** Lisp Hooks
:PROPERTIES:
:CUSTOM_ID: lisp-hooks
:END:

Entering and exiting Uniline invokes so-called "hooks":

- =uniline-mode-hook= ran whenever the command =uniline-mode= is called.
- =uniline-mode-on-hook= ran whenever entering =uniline-mode=.
- =uniline-mode-off-hook= ran whenever exiting =uniline-mode=.

Some users may want to always have =picture-mode= along with
=uniline-mode=. This makes sense to benefit from the best of both
packages. They just add this line to their =.emacs= file:

#+begin_src elisp
(add-hook 'uniline-mode-hook 'picture-mode)
#+end_src

Other users may want to set the variable =line-spacing= to zero when
operating under =uniline-mode=. This makes sense, as drawings are
visually more pleasant with this setting. They write this in their
=.emacs= file:

#+begin_src elisp
(defun no-line-spacing ()
  (setq-local line-spacing 0.0))
(add-hook 'uniline-mode-on-hook 'no-line-spacing)
#+end_src

Note: defining the =no-line-spacing= function may be avoided using a
=lambda=. But the =add-hook= function recommends a =defun=.

Look here [[#use-case---mixing-fonts][Use case - mixing fonts]] for a more advanced use of hooks,
suggested by a Uniline user.
...
#+end_example
↓
** Compatibility with abbrev
:PROPERTIES:
:CUSTOM_ID: compatibility-with-abbrev
:END:
With the builtin =abbrev= package, type a word, then =C-x '=, and the word
is replaced by its associated text.

Of course, you can use =abbrev= (or any other similar package like
=yasnippet= or =tempel=) under Uniline.

Now Uniline adds the ability to expand a word into a rectangular
text. This is achieve through the =uniline-abbrev-hook= function
associated to each abbreviation created by Uniline. See [[#abbrev][Abbrev]].

If you display all the abbreviations with the =M-x list-abbrevs=
command, you will see something like:

[[file:images/abbrev-editting.png]]

#+begin_example

   ╭────the name of the abbreviation
   │     ╭─the number of times this abbreviation has been invoked so far
...
╰────────────────────────────────────────────────────────▲─────────╯
                                                         │
  the hook to make the insertion rectangular╶────────────╯

#+end_example

You can edit that: the name of the abbrev, or the rectangle to insert.
Then save the result for future sessions with =C-x C-s=.  Just keep the
Uniline hook. It was set by Uniline when creating the abbrev with
=<insert> a=.

The rectangle begins with a newline which will be ignored, but which
makes the rectangle readable and editable.
...
** Emacs on MacOS
:PROPERTIES:
:CUSTOM_ID: emacs-on-macos
:END:
The MacOS keyboards are different from the Linux or Windows
ones. Particularly they are missing the insert and the tilde keys.

The insert key is used for launching sub-menus, implemented in Hydra
or Transient. The key to launch those sub-menus is customizable. See
[[#insert-key][Insert key]].

The control insert key (=C-INS=) is also the recommended binding to
=uniline-mode= for entering and exiting Uniline. See [[#installation][Installation]]. If
you, instead, want to bind =uniline-mode= to =C-£= (Pound sterling,
not so used nowadays), then put that in your =.emacs= file:
...
The tilde =~= character is not always easy to type under
MacOs. It is usually entered by typing =Alt-n SPC=. (Typing
=Alt-n n= gives =ñ=, typing =Alt-n o= gives =õ= and so on).

However, under Emacs the =Alt= keys are usually interpreted as =Meta=,
which defeats the tilde typing. One way to circumvent that, is to give
back the =Alt= key at the right to MacOS, and keep the Emacs meaning
(ie. =Meta=) for the =Alt= key at the left.

To do so:

#+begin_src example
M-x customize-variable ns-right-alternate-modifier
#+end_src

and set this variable to =nil= (by default it is set to =left=).

Then enter tilde with the right =Alt= key.

You may also look at [[#keyboard-bindings][Keyboard bindings]] for a way to bind tilde =~= and
other keys.

The default monospaced font on MacOS used to be =Menlo=. It was
superseded by =SF-Mono=. Uniline cannot support those fonts, as their
licensing is quite restrictive (=SF-Mono= much more than =Menlo=).

You may still use them on your own. Just customize the =default-face= or
type something like:
...
=Menlo= supports the Unicode characters used by Uniline. Although the
blocks-characters appear disconnected from one another, even with
=line-spacing= set to =nil=.
...
The API is usable programmatically. The Uniline functions work even
when the buffer is not in =uniline-mode=.
...
- =uniline-draw-inner-rectangle=
- =uniline-draw-outer-rectangle=
- =uniline-overwrite-inner-rectangle=
- =uniline-overwrite-outer-rectangle=
...
Many packages declare their entry-point as "autoload":

#+begin_src elisp
;;;###autoload
(define-minor-mode minautorus …)

;; minautorus-mode is now an autoloaded function
#+end_src

Uniline cannot do that. If it did, invoking =uniline-mode= would load
=uniline-core.elc= and nothing else (because =uniline-mode= is defined in
the =uniline-code.el= file). To properly function, uniline need one of
the two interfaces to be loaded: =uniline-hydra.elc= or
=uniline-transient.elc=.

Actually, this is what =use-package= does behind the scene:

=(use-package uniline-hydra …)=

is translated to:

=(autoload 'uniline-mode "uniline-hydra" nil t)=

You can do that manually in your =.emacs= initialization file.
...
- =boxes=: draws artistic boxes around text, with nice looking unicorns,
  flowers, parchments, all in ASCII art. Melpa. Interface to external
  utility.

- =org-drawio=: a bridge between the Draw.Io editor and =Emacs=, producing
  drawing similar to those of =Uniline=, but in =.svg=.

- =syntree=: draws ASCII trees on-the-fly from description. Melpa.
...
- =show-font=: show font features in a buffer. Elpa.

- =ob-svgbob=: convert your ascii diagram scribbles into happy little
  SVG. Melpa.

- =el-easydraw=: a full featured SVG editor right inside your
  =Emacs=. Impressive! GitHub.

- =ascii-draw=: like =asciiflow= with Unicodes.

- =ibm-box-drawing-hydra.el=: keyboard interface to insert UNICODE
  box-drawing characters one at a time. GitHub Gist.
...
- =rcd-box=: create tables surrounded by box-drawing characters from
  Lisp descriptions. At hyperscope.link.
...
- =ob-pikchr.el=: integration of the excellent =pikchr= in Emacs Org-Mode.
...
- =asciiflow=: (not =Emacs=) draw on the web, then copy-paste your UNICODE
  text.
...
- =excalidraw.com=: inline drawing, but not in Unicode or Ascii. Web
  interface.

- =vim drawit ascii diagrams=: in Vim, in ASCII.

- =MarkDeep=: (Casual Effects): write in Markdown, render on the Web on
  the fly. Impressive! Uniline may be used to author part of the
  Markdown source.

- =image-to-ascii=: turns a photo into ASCII. At www.asciiart.eu.
...
- =pikchr=: describe objects and their relationship in Markdown, render
  in Unicode. Very precise textual description of graphic obbjects
  layout.
...
- λ.ςζ szkl (GitHub) instantly pinpointed a debug log left-over.
...
- jian-lin (on GitHub) diagnosed and fixed the =uniline--color-hint=
  compilation problem
